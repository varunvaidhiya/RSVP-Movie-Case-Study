...
-- 1007 movies produced in the USA or India in the year 2019
...
-- Type your code below:
SELECT     genre, Count(x.id) AS number_of_movies
FROM       movie       AS x
INNER JOIN genre       AS y
where      y.movie_id = x.id
GROUP BY   genre
ORDER BY   number_of_movies DESC limit 1 ;

-- Drama has the highest with 4285
...
SELECT genre,Count(a.id) AS movie_count FROM   movie AS a
	INNER JOIN genre AS b
      ON b.movie_id = a.id
...
-- Type your code below:
SELECT median_rating, Count(*) AS movie_count FROM movie AS a
...
WITH VOTES_SUMMARY 
   AS (
       SELECT languages, SUM(total_votes) AS VOTES FROM MOVIE AS a
          INNER JOIN RATINGS AS b 
            ON b.MOVIE_ID = a.ID
